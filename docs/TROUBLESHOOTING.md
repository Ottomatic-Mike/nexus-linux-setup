...
prefers that signature and otherwise uses an extreme tall-display ratio. It
...
On Plasma, the local workaround appears as:

```text
/etc/systemd/system/nexus.service.d/10-wait-for-plasma.conf
```

If Nexus was already running before this helper was installed, run:
