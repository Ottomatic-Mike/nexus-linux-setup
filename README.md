...
- detects whether `nct6775` exposes a real NCT67xx/NCT679x PWM controller and,
  ...
  cooling hardware;
- on KDE Plasma, makes the current Nexus service wait for the active Plasma
...
The local workaround waits for the target user's Plasma process before starting
...
The `upstream/` directory contains a sanitized reproduction, implementation
contracts, tests to add, a ready-to-paste PR description and submission steps.

The public umbrella repository currently says its component repositories,
including `nexus-service` and `nexus-web`, are private for now. For that reason
this repository intentionally does **not** fabricate source paths or a fake
source diff against code that is not publicly reviewable.

See [`upstream/SUBMIT.md`](upstream/SUBMIT.md).
